...
$ make install
```
Note that this command assumes that `/usr/local/bin` is on the `PATH` and that files can be moved to it, so it is recommened to only use `make install` if using a Linux OS. 
