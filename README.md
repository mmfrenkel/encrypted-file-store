...
### I. Generation of Cryptographic Keys

The cryptographic keys used in the AES encryption and HMAC hashing steps described below are derived from the user-provided password. Upon submission, the user's password is iteratively hashed 10,000 times using SHA-256 hashing to produce the cryptographic key `k`. A second key, `k`<sub>1</sub>, is derived from the first key `k` after another 10,000 iterations of SHA-256. This second key is generated to ensure that HMACs for archive integrity are not derived from the same key as HMAC codes for file integrity (see more below).
...
When a new archive is created, an HMAC is generated using the new archive's name as `t` and the second cryptographic key, `k`<sub>1</sub>. This HMAC is sent to the `.metadata` file for the archive, such that it can be re-read from the `.metadata` file from that archive in the future and compared against the HMAC created from the password submitted each time a user attempts to add, extract or delete files. This means that only users that know the password that was used in the initial creation of the archive can make edits to the archive (though anyone may still list the files). Users with the incorrect passwords will get an integrity alert, asking for a different password.
...
In order to make sure that the actual content of an individual encrypted file itself is not corrupted or tampered with, an HMAC is generated for each file using the file's ciphertext as `t` and the cryptographic key. This HMAC is appended to the ciphertext and stored in the encrypted file (i.e., ct || HMAC). This means that when a person attempts to extract their encrypted file from the archive, an integrity check can be made by comparing the initial HMAC assigned to an encrypted file (which is read in from the file on decryption) with the HMAC that is recomputed from ciphertext read in from the filestore. The integrity check fails if the two HMAC hashes do not match. Since users must have submitted the correct password in order to make it to the extraction step (see `i.`), this integrity check fails when a file has been corrupted in storage. Users are alerted of any such integrity violation.  
